      ...
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navItems.map((item, idx) => (
            <a key={idx} href={item.link} className="hover:text-violet-400 transition p-2">
              {item.name}
            </a>
          ))}
        </div>
                  ...
                  {user?.plan && (
                    ...
                    >
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold text-white ${
                        user.plan === 'Premium' ? 'bg-gradient-to-r from-purple-500 to-pink-500'
                        : user.plan === 'Standard' ? 'bg-gradient-to-r from-blue-500 to-indigo-500'
