      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        {[...Array(20)].map((_, i) => (
          <div key={i} className="absolute w-1 h-1 bg-violet-400/30 rounded-full animate-float"
            style={{ left: `${(i * 37) % 100}%`, top: `${(i * 53) % 100}%`, animationDelay: `${i * 0.3}s`, animationDuration: `${8 + (i % 5)}s` }}
          />
        ))}
      </div>
        ...
        <div className="w-full max-w-lg">
          <div className={`transition-all duration-1000 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="text-center mb-8">
              ...
              <p className="text-slate-500 text-sm">Create your account and find your perfect match</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm border border-white/10 rounded-3xl p-8 hover:border-violet-500/30 transition-all duration-300">
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                      ...
                      className={`w-full bg-white/5 border ${errors.name ? 'border-red-500/70' : 'border-white/10'} rounded-xl pl-12 pr-4 py-3.5 text-white placeholder-slate-500 focus:outline-none focus:border-violet-500/70 focus:bg-violet-500/5 transition-all`}
                    />
                  </div>
                  {errors.name && <p className="text-red-400 text-sm mt-1.5 flex items-center gap-1"><span className="w-1 h-1 bg-red-400 rounded-full" />{errors.name}</p>}
                </div>
                ...
                <div>
                  ...
                  {errors.email && <p className="text-red-400 text-sm mt-1.5 flex items-center gap-1"><span className="w-1 h-1 bg-red-400 rounded-full" />{errors.email}</p>}
                </div>
                  ...
                  {form.password && (
                    <div className="mt-2">
                      <div className="flex gap-1 h-1">
                        {[1, 2, 3, 4].map((level) => (
                          <div key={level} className={`flex-1 rounded-full transition-all duration-300 ${level <= strength ? strengthColor[strength] : 'bg-white/10'}`} />
                        ))}
                      </div>
                      <p className="text-xs text-slate-500 mt-1">
                        Strength:{' '}
                        <span className={`font-medium ${strength <= 1 ? 'text-red-400' : strength === 2 ? 'text-yellow-400' : strength === 3 ? 'text-cyan-400' : 'text-green-400'}`}>
                          {strengthLabel[strength]}
                        </span>
                      </p>
                    </div>
                  )}
                  {errors.password && <p className="text-red-400 text-sm mt-1 flex items-center gap-1"><span className="w-1 h-1 bg-red-400 rounded-full" />{errors.password}</p>}
                </div>
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input type={showConfirm ? 'text' : 'password'} value={form.confirm}
                      ...
                      className={`w-full bg-white/5 border ${errors.confirm ? 'border-red-500/70' : form.confirm && form.confirm === form.password ? 'border-green-500/50' : 'border-white/10'} rounded-xl pl-12 pr-12 py-3.5 text-white placeholder-slate-500 focus:outline-none focus:border-violet-500/70 focus:bg-violet-500/5 transition-all`}
                    />
                  ...
                  {errors.confirm && <p className="text-red-400 text-sm mt-1.5 flex items-center gap-1"><span className="w-1 h-1 bg-red-400 rounded-full" />{errors.confirm}</p>}
                </div>

                <div>
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <div onClick={() => setAgreed(!agreed)}
                      className={`mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${agreed ? 'bg-gradient-to-br from-violet-600 to-cyan-600 border-transparent' : 'border-white/20 group-hover:border-violet-500/50'}`}>
                  ...
                  {errors.terms && <p className="text-red-400 text-sm mt-1.5 flex items-center gap-1"><span className="w-1 h-1 bg-red-400 rounded-full" />{errors.terms}</p>}
                </div>
            ...
            <div className="flex items-center justify-center gap-6 mt-8 text-slate-500 text-xs">
              <span className="flex items-center gap-1.5"><Sparkles className="w-3 h-3 text-violet-400/70" /> Free Forever</span>
              <span className="w-1 h-1 bg-slate-600 rounded-full" />
              <span className="flex items-center gap-1.5"><Star className="w-3 h-3 text-violet-400/70" /> No Credit Card</span>
              <span className="w-1 h-1 bg-slate-600 rounded-full" />
